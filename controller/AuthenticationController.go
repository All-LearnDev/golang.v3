...
import (
	"net/http"
	"projects/entitys"
	repositorys "projects/repositorys/author"
	"projects/utils"
...
func Login(c echo.Context) error {
	...
	user = repositorys.FindUserByUserName(username)
	...
	repositorys.SaveRefreshToken(refreshToken)
	if er != nil {
		return er
	} else {
...
func Register(c echo.Context) error {
	...
	user := repositorys.FindUserByEmail(email)
	if user.Name != "" {
		return c.String(http.StatusBadRequest, "Email exits in DB")
	}
	// Register user
	var newUser entitys.JUser
	newUser = repositorys.AddJUser(username, email, password)
...
func RenewToken(c echo.Context) error {
	...
	if utils.ValidToken(refreshToken) {
		if utils.ExpiredToken(refreshToken) {
			refreshTokenObject := repositorys.FindRefreshTokenByToken(refreshToken)
			if refreshTokenObject.UserName != "" {
				...
				// Save refreshToken to DB:
				repositorys.SaveRefreshToken(return_refresh_token)
			}
